...
//    NSLog(@"setDialRange frame = %@", NSStringFromCGRect(frame));
...
//    NSInteger label_x = point.x - (boundingBox.width / 2);
    ...
    CGFloat label_x = self.majorTickLength + (boundingBox.width / 2);
    
    // We want the label to be centered on the specified x value
    NSInteger label_y_offset = point.y - (boundingBox.height / 2);
...
- (void)drawMinorTickWithContext:(CGContextRef)context
    ...
    CGContextMoveToPoint(context, point.x-length/2-_minorTickLength/2, point.y);
...
- (void)drawMajorTickWithContext:(CGContextRef)context
    ...
    CGContextMoveToPoint(context, point.x-length/2-_majorTickLength/2, point.y);
    CGContextAddLineToPoint(context, point.x + length, point.y);
...
- (void)drawTicksWithContext:(CGContextRef)context atX:(long)x
{
    ...
    if ([self isMajorTick:x]) {
        ...
        //
//        int value = (point.y - self.toping) / self.minorTickDistance + _minimum;
//
...
- (void)drawTicksWithContext:(CGContextRef)context atX:(long)x atY:(long)y
{
...
//    NSLog(@"drawRect frame = %@\n", NSStringFromCGRect(rect));
    ...
    //决定中间线长度
    [self setMajorTickLength:rect.size.width/4];
    [self setMinorTickLength:rect.size.width/6];
    
    // Add the tick Marks
    for (long i = self.toping; i < rect.size.height; i += self.minorTickDistance) {

        
        // After
        if (i > (self.frame.size.height - self.toping))
            break;

        // Middle
        else{
            [self drawTicksWithContext:context atX:(rect.size.width/2) atY:i];
        }
...
- (BOOL)isMajorTick:(long)x {
    ...
    long tick_number = (x - self.toping) / self.minorTickDistance;
