...
public class SocketThread{
    ...
    private SocketThread (Handler handler,Context context)
    {
//    	this.IP=IP;
//    	this.Port = Port;
	...
	public static SocketThread getSingleInstance(Handler handler, Context context) {
		if (Single == null) {
			synchronized (SocketThread.class) {
				if (Single == null) {
					Single = new SocketThread(handler, context);
				}
			}
		}
		return Single;
			...
			public void run() {
				...
				while(true){
					try {
						...
						mSocket.connect(new InetSocketAddress(IP, Port), 6000);
						delay(500);
...
//					int lengh = socketRecvData(recvBytes);
//					if (lengh > 0 ) {
						Msg = Message.obtain(mHandler);
//						Msg.obj = new String(recvBytes, 0, lengh);
						Msg.what = 520;
						mHandler.sendMessage(Msg);
//					}			
	...
	public void socketClose()
	{
		...
		if (DEBUG_MODE)
			Log.v("SocketThread", "begin SocketClose");
		
		if (mInStream != null) {
			try {
				mInStream.close();
			} catch (IOException e) {
				Log.v("SocketThread", "mInStream.close:" + e.getMessage());
			}
		}
		...
		if (mOutStream != null) {
			try {
				...
				Log.v("SocketThread", "mOutStream.close:" + e.getMessage());
			}
		}
		if (mSocket != null) {
			try {
		...
		if (DEBUG_MODE)
			Log.v("SocketThread", "done Socket closed");
	}
	
	public void socketSendData(byte[] inData)
	{
		...
		sendRecvHandler.post(new Runnable() {
			...
			public void run() {
				...
				try {	
					...
					Log.v("SocketThread", "mOutStream.write:" + e.getMessage());
					socketClose();
					Msg = Message.obtain(mHandler);
...
//				}
	...
	public int socketRecvData(byte[] outData)
	{
		...
		try {
			...
			socketClose();
			Log.v("SocketThread", "mInStream.read:" + e.getMessage());
			return -3;
		}
